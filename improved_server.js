...
const MOCK_OPENAI = !!process.env.MOCK_OPENAI; // when true, return deterministic mock JSON
if (!OPENAI_KEY && !MOCK_OPENAI) console.warn("WARNING: OPENAI_KEY not set in env/replit secrets.");
...
function buildMockOutput() {
  return {
    hero: { title: "Юридическая помощь — mock", subtitle: "Быстро и надёжно (mock)", cta: "Связаться" },
    benefits: [
      { title: "Опыт", text: "10+ лет практики" },
      { title: "Результат", text: "Гарантированный подход" }
    ],
    process: [
      { step_title: "Шаг 1", step_text: "Консультация" },
      { step_title: "Шаг 2", step_text: "Подготовка документов" }
    ],
    faq: [{ q: "Как обратиться?", a: "Через форму на сайте" }],
    seo: { title: "Юридические услуги — mock", description: "Краткое описание (mock)." }
  };
}
...
app.post("/generate", async (req, res) => {
  try {
    if (!checkToken(req)) {
      console.info("Auth failed - token invalid or missing");
      return res.status(401).json({ error: "Unauthorized: invalid token" });
    }

    const { brief = "", page_type = "invest", sessionId = "session-1" } = req.body;
    if (!brief) {
      console.info("Empty brief received");
      return res.status(400).json({ error: "Empty brief" });
    }

    if (MOCK_OPENAI) {
      const outMock = buildMockOutput();
      inMemoryStore[sessionId] = { data: outMock, createdAt: Date.now() };
      return res.json(outMock);
    }

    if (!OPENAI_KEY) {
      console.error("OPENAI_KEY missing and not in mock mode");
...
app.get("/export", async (req, res) => {
  try {
    if (!checkToken(req)) {
      console.info("Export auth failed");
