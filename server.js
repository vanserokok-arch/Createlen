...
// POST /generate -> calls OpenAI and stores result in memory or queues job
    ...
    if (async) {
      try {
        ...
        return res.status(500).json({ 
          error: 'Failed to queue job. Ensure DATABASE_URL and REDIS_URL are configured.',
          details: err.message 
        });
      }
    }
...
// GET /status/:sessionId -> get async job status
app.get("/status/:sessionId", async (req, res) => {
  try {
    ...
    try {
      ...
      return res.status(500).json({ 
        error: 'Failed to check status. Ensure DATABASE_URL is configured.',
        details: err.message 
      });
    }
  } catch (err) {
